...
export default function FormStep() {
  ...
  const isCurrentStepValid = () => {
    switch (currentStep) {
      ...
      case 4:
        return formData.frontendItems?.some(item => !!item.framework && !!item.version);

      case 5:
        return formData.backendItems?.some(item =>
          !!item.language && !!item.languageVersion && !!item.framework && !!item.frameworkVersion
        );

      case 6:
        return formData.webServerItems?.some(item => !!item.server && !!item.version);
