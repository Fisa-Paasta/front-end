...
export default function AdminSidebar({ onFilter, active }: AdminSidebarProps) {
  const menuItems: { label: string; status: string }[] = [
    ...
    { label: '로그 보기', status: 'LOG_VIEW' },
    { label: '삭제된 요청', status: '삭제됨' },
  ];

  const getButtonClassName = (label: string, status: string) => {
    const isActive = active === label;
    const statusClass = statusClasses[status] ?? '';
    
    if (isActive) {
      return `w-full text-left cursor-pointer px-2 py-1 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-primary ${statusClass} font-bold`;
    }
    
    return 'w-full text-left cursor-pointer px-2 py-1 rounded transition-colors focus:outline-none focus:ring-2 focus:ring-primary hover:text-primary hover:bg-gray-100 dark:hover:bg-gray-700';
  };
          ...
          {menuItems.map(({ label, status }) => (
            <li key={label}>
              <button
                ...
                className={getButtonClassName(label, status)}
